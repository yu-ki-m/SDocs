...
describe("表示", () => {
    ...
    describe("メイン", () => {
        it("初期表示されていること(props,slot)", async () => {
            ...
            expect((wrapper.find('label>input[id="editor_docs-name"]').element as HTMLInputElement).value).toBe(props.docsName);
            ...
            expect((wrapper.find('label>input[id="editor_docs-version"]').element as HTMLInputElement).value).toBe(props.docsVersion);
            ...
            expect((wrapper.find('label>input[id="editor_docs-id"]').element as HTMLInputElement).value).toBe(props.docsId);
        });
        ...
        it("Docs名を入力", async () => {
            ...
            // * Assert
            expect((wrapper.find('label>input[id="editor_docs-name"]').element as HTMLInputElement).value).toBe("変更後のDocs名");

            /** emitに渡される値 */
            let emitValue:DocsBaseInfoInterface|undefined = wrapper.emitted<DocsBaseInfoInterface[]>('input')?.pop()?.pop() ;
        ...
        it("Docsバージョンを入力", async () => {
            ...
            // * Assert
            expect((wrapper.find('label>input[id="editor_docs-version"]').element as HTMLInputElement).value).toBe("変更後のDocsバージョン");

            /** emitに渡される値 */
            let emitValue:DocsBaseInfoInterface = wrapper.emitted<any>('input')?.pop()?.pop() ;
        ...
        it("DocsのIDを入力", async () => {
            ...
            // * Assert
            expect((wrapper.find('label>input[id="editor_docs-id"]').element as HTMLInputElement).value).toBe("変更後のDocsID");
