...
export default class TableContent{
    ...
    addRecord = (insertPostion:number)=>{
        // 挿入元を引用しディープコピー
        let newRecord:RecordContent = structuredClone((this.records[insertPostion]));
        // コピー新規作成したレコードを初期化
        ...
        newRecord.cells = newRecord.cells.map(
        ...
        this.records.forEach(
            (recordContent:RecordContent)=>{
                if(recordContent.cells.length < 10){
                    recordContent.cells.splice(insertPostion+1,0,new CellContent(this.tableContentModule.uuid.getUniquId(),CELL_TYPE.QUILL,""));
                }
            }
        )
    }
    ...
    replacePositionRow = (replacePostion:number,replaceTargetPostion:number)=>{
        ...
        this.records.forEach(
            (recordContent:RecordContent)=>{
                let temp = recordContent.cells[replacePostion];
